...
(** [run_powershell fmt] will execute [powershell -Command "fmt"].
    ...
    @param escape
      (defaults to {!Fun.id}) allows to escape [fmt] because the calling shell
      (usually [cmd]) might interpret unwanted things in [fmt]. This might help
      embedding readable powershell code. *)
...
(** Remove the system attribute on a path. Might be useful to copy data across
    images when building Docker images. *)
...
module Cygwin : sig
  ...
  *)
  ...
  (** Install Cygwin with CygSymPathy and optionally msvs-tools, and [extra]
      Cygwin packages (first in a separate Docker image). Sets the
  ...
  (** [cygwin_packages ?extra ()] is the list of the base development tools for
      the OCaml Cygwin port. *)

  val mingw_packages : string list
  (** [mingw_packages] is the list of base development tools for the Caml mingw
      port. *)

  val msvc_packages : string list
  (** [msvc_packages] is the list of base development tools for the Caml MSVC
      port. *)

  val install_ocaml_for_windows :
    ?cyg:cyg -> ?version:string -> unit -> string list * t
  (** [install_ocaml_for_windows ()] returns the list of Cygwin packages
      dependencies, and the installation instructions for OCaml for Windows.
  ...
  module Git : sig
    ...
    (** Configure the git name and email variables to sensible defaults.
        ...
        @param repos
          A list of paths to Git repos to mark as safe directories. Defaults to
          the default location of the opam-repository. *)
  end
end
...
module Winget : sig
  ...
  (** Install winget from a released build (first in a separate Docker image).
      The optional [winget_version] specifies a Git tag. *)
  ...
  (** [dev_packages distro ?extra ()] will install the base development tools.
