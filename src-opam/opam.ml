...
let windows_mingw_opam2 ?win10_revision ?winget ?(labels = []) ?arch
  ...
  let opams_image =
    ...
    @@ Windows.Cygwin.install_cygwin ~aslr_off:(version = `V1809) ~extra:packages ()
  ...
  let ocaml_for_windows =
    ...
    Windows.Cygwin.install_cygwin ~aslr_off:(version = `V1809) ~extra:packages () @@ setup
  in
...
let windows_msvc_opam2 ?win10_revision ?winget ?(labels = []) ~opam_hashes
  ...
  let cygwin_msvc_image =
    ...
    @@ Windows.sanitize_reg_path ()
    @@ vs_build_tools
    @@ Windows.Cygwin.install_cygwin ~msvs_tools:true ~aslr_off:(version = `V1809) ~extra:packages ()
  in
  let opams_image =
