...
let resolve_alias (d : t) : distro =
  ...
  | ( `Alpine
        ...
        | `V3_19 | `V3_20 | `V3_21 | `V3_22 | `V3_23 | `V3_24 )
    | `Archlinux `Latest
...
let distro_arches ov (d : t) =
  ...
  | `Debian `V12, ov when OV.(compare Releases.v4_03_0 ov) = -1 ->
      [ `I386; `X86_64; `Aarch64; `Aarch32; `Ppc64le ]
...
let tag_of_distro (d : t) =
  ...
  | `OracleLinux `V10 -> "oraclelinux-10"
...
let distro_of_tag x : t option =
  ...
  | "oraclelinux-10" -> Some (`OracleLinux `V10)
...
let bubblewrap_version (t : t) =
  ...
  | `CentOS `V9 -> Some (0, 6, 3)
  | `CentOS `V10 -> Some (0, 10, 0)
...
let base_distro_tag ?(arch = `X86_64) d =
  ...
  | `CentOS v ->
      let tag =
        ...
        | `V10 -> "stream10"
      in
      ( (match v with `V9 | `V10 -> "quay.io/centos/centos" | _ -> "centos"),
        tag )
  | `Fedora v ->
      let tag =
        match v with
  ...
  | `OracleLinux v ->
      let tag =
        match v with `V7 -> "7" | `V8 -> "8" | `V9 -> "9" | `V10 -> "10"
      in
      ("oraclelinux", tag)
