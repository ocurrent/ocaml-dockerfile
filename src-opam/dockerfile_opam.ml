...
let header ?arch ?maintainer img tag =
  ...
  let shell =
    match arch with
    | Some arch when OV.arch_is_32bit arch && img <> "windows" ->
  ...
  in
  let escape =
    let windows_img, _ = Dockerfile_distro.(base_distro_tag (`Windows `V20H2)) in
    if img = windows_img then parser_directive (`Escape '`') else empty
  in
  escape @@
...
let gen_opam2_distro ?(clone_opam_repo=true) ?arch ?labels d =
  ...
  | `Cygwin -> cygwin_opam2 ?labels ?arch ~tag ~distro ()
  in
